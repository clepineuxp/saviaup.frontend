...
El contrato de productos usa `Product { id, type, name, description, image, category, salePrice, preparationTimeMinutes, isInventoryTracked, isActive, recipe, variations, comboGroups, createdAt, updatedAt }`. `type` es `NORMAL | COMBO`; crear y actualizar envían el `categoryId` y los demás campos editables. `salePrice` es positivo para combos y normales sin variaciones, pero es `null` cuando un normal tiene variaciones porque siempre se vende eligiendo una de ellas. Un combo exige grupos con opciones basadas en productos normales. El listado usa `{ items, page, pageSize, totalCount, totalPages }` y los filtros se resuelven en backend. La eliminación responde `204`, requiere confirmación explícita y nunca se aplica de forma optimista.
