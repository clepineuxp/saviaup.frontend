...
La primera entrega de inmediato el layout, estilos, categorías, productos, variaciones, precios y composición de combos. Los combos muestran cada sección por su nombre configurado, si la selección es única, múltiple o fija, los límites aplicables, las opciones disponibles, las unidades incluidas y cualquier recargo o descuento. La cuadrícula de productos crece con el ancho disponible y el carrusel superior ofrece un menú desplegable para acceder directamente a cualquier categoría. La segunda entrega las imágenes WebP comprimidas dentro del JSON y se consume secuencialmente por categoría, de modo que el menú permanece usable mientras aparecen las imágenes. El backend limita inicialmente cada imagen a 640 px y aplica compresión adaptativa hasta un objetivo de 64 KB, reduciendo calidad y dimensiones únicamente cuando continúa pesada. No se requieren rutas públicas de archivos en el ingress.
