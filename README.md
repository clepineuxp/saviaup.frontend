...
- `TableRealtimeClient` conecta únicamente durante el ciclo de vida de la feature, obtiene un JWT vigente en cada reconexión y mantiene reintentos con espera máxima de 30 segundos. Al recuperar foco, visibilidad o conectividad valida el canal mediante `Ping`; solo después de reconstruir una conexión perdida vuelve a consultar silenciosamente el snapshot REST para cubrir eventos que pudieron perderse mientras la PWA estuvo suspendida.
- En móvil, arrastrar hacia abajo desde el inicio de la página de ventas comprueba la conexión mediante un ping de SignalR. Si continúa activa no consulta nuevamente el snapshot; si se perdió, intenta reconectar y sincroniza las mesas en segundo plano, sin reemplazar el plano por el estado de carga. El plano admite zoom entre 50 % y 175 % y conserva el nivel al reconstruirse después de una venta: escritorio muestra controles, mientras tablet y móvil usan el gesto de dos dedos. Solo se escalan las mesas y sus coordenadas, sin afectar métricas, barras ni diálogos.
