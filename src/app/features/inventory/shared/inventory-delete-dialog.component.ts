  ...
  template: `
    <div class="backdrop">
      <button
        ...
        (click)="cancel()"
      ></button>
      <section role="alertdialog" aria-modal="true" aria-labelledby="inventory-delete-title">
        <span class="icon" aria-hidden="true">!</span>
        <h2 id="inventory-delete-title">{{ title() }}</h2>
        <p>{{ description() }}</p>
        <small>{{ 'inventory.delete.warning' | translate }}</small>
        ...
        <footer>
          ...
          @if (error()?.code === inUseCode()) {
            <app-ui-button
              type="button"
              [loading]="submitting()"
              (click)="deactivateRequested.emit()"
            >
              {{ 'inventory.delete.deactivateInstead' | translate }}
            </app-ui-button>
          } @else {
            <button
              class="danger"
            ...
            >
              {{ 'inventory.delete.confirm' | translate }}
            </button>
          }
        </footer>
      </section>
...
export class InventoryDeleteDialogComponent {
  ...
  readonly inUseCode = input.required<string>();
