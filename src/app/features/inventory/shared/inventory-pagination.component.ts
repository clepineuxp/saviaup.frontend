...
@Component({
  ...
  template: `
    @if (totalPages() > 1) {
      <nav class="pagination" [attr.aria-label]="'inventory.pagination.label' | translate">
        <button type="button" [disabled]="page() <= 1" (click)="pageChanged.emit(page() - 1)">
          ‹ <span>{{ 'inventory.pagination.previous' | translate }}</span>
        </button>
        <p>
          {{
        ...
        >
          <span>{{ 'inventory.pagination.next' | translate }}</span> ›
        </button>
      </nav>
    }
  `,
