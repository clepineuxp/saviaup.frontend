...
describe('HttpProductRepository', () => {
  ...
  it('loads a bounded server-filtered page of active normal products for combo options', async () => {
    get.mockReturnValueOnce(
      of({ items: [product], page: 1, pageSize: 50, totalCount: 1, totalPages: 1 }),
    );

    const result = await firstValueFrom(repository.listComboCandidates('limonada grande'));

    expect(result.map((candidate) => candidate.id)).toEqual(['product-1']);
    expect(get).toHaveBeenCalledWith(API_ENDPOINTS.products.root, {
      params: {
        page: 1,
        pageSize: 50,
        search: 'limonada grande',
        type: 'NORMAL',
        includeInactive: false,
      },
    });
